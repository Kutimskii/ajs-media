...
class Timeline {
  ...
  subscribeEvents() {
    ...
    this.container.querySelector(".form").addEventListener("submit", e => {
      ...
      this.post.textContent = this.container.querySelector(".form").querySelector("input").value;
      this.container.querySelector(".content").insertAdjacentElement("beforeend", this.post);
      this.container.querySelector(".form").querySelector("input").value = "";
      this.getGeolocation();
    });
    this.container.querySelector(".modal__form").addEventListener("submit", e => {
      e.preventDefault();
      if (this.container.querySelector(".modal__form").checkVisibility()) {
        const coordinates = document.createElement("div");
        coordinates.classList.add("coordinates");
        coordinates.textContent = this.container.querySelector(".modal__form").querySelector("input").value;
        this.post.insertAdjacentElement("beforeend", coordinates);
        this.container.querySelector(".modal__form").querySelector("input").value = "";
        this.showModal();
      }
    });
    this.container.querySelector(".modal__form").querySelector(".btn_reset").addEventListener("click", e => {
      e.preventDefault();
      this.showModal();
    });
    this.container.querySelector(".btn__audio").addEventListener("click", e => {
      e.preventDefault();
      this.container.querySelector(".btn__video").classList.remove("active");
      this.container.querySelector(".btn__audio_stop").classList.add("active");
      this.recordAudio();
    });
    this.container.querySelector(".btn__video").addEventListener("click", e => {
      e.preventDefault();
      this.container.querySelector(".btn__audio").classList.remove("active");
      this.container.querySelector(".btn__video_stop").classList.add("active");
      this.container.querySelector(".btn__video").classList.add("record");
      this.recordVideo();
    });
  }
  showModal() {
    this.container.querySelector(".modal__container").classList.toggle("active");
  ...
  async recordAudio() {
    ...
    recorder.addEventListener("stop", () => {
      ...
      this.container.querySelector(".btn__audio_stop").classList.remove("active");
      this.container.querySelector(".content").insertAdjacentElement("beforeend", this.post);
    });
    recorder.start();
  ...
  async recordVideo() {
    ...
    recorder.addEventListener("stop", () => {
      ...
      this.container.querySelector(".btn__video_stop").classList.remove("active");
      this.container.querySelector(".content").insertAdjacentElement("beforeend", this.post);
    });
    recorder.start();
